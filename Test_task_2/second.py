...
res = []  # Глобальная переменная - список


def func(data_set: dict, start: int):  # Функция для вывода достижимых вершин
    global res  # Объявляем глобальную переменную
    res.append(start)
    ...
    for item in data_set.get(start):  # Получаем значения по ключу
        if item not in res:  # Проверяем наличие элемента в результирующем списке 
            if item in keys:  # Проверяем, является ли это значение ключом в словаре
                func(data_set, item)  # Вызываем функцию с тем же словарем, но ключ используем новый
            else:
                res.append(item)  # Добавляем значение в список
    return res


for i in func(data, 1):  # Вызываем функцию 
    print(i)
